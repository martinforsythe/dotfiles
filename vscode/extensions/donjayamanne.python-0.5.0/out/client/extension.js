...
function activate(context) {
    ...
    context.subscriptions.push(vscode.languages.registerRenameProvider(PYTHON, new renameProvider_1.PythonRenameProvider()));
    ...
    if (pySettings.get('formatting.formatOnSave') === true) {
        vscode.window.showInformationMessage(`'python.formatting.formatOnSave' has been deprecated. Please use 'editor.formatOnSave' instead`, 'Help').then(item => {
            if (item === 'Help') {
                vscode.commands.executeCommand('python.displayHelp', constants_1.Documentation.Formatting.FormatOnSave);
            }
        });
    }
}
