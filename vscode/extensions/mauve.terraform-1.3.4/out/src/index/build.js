...
function terraformSectionFromItemNode(uri, node) {
    ...
    const requiredVersion = ast_1.getStringValue(requiredVersionVal, "", { stripQuotes: true });
    const start = createPosition(node.Keys[0].Token.Pos);
    const end = endPosFromVal(node.Val);
    return new file_index_1.TerraformSection(requiredVersion, new location_1.Location(uri, new range_1.Range(start, end)), node);
}
            ...
            if (node.Keys.length === 1) {
                ...
                if (path.length === 2) {
                    if (node.Keys[0].Token.Text === "terraform") {
                        let terraformSection = terraformSectionFromItemNode(uri, node);
