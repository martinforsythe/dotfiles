...
exports.default = gulp.task('build:icons', cb => {
    let contents;
    const fileNames = fs.readdirSync(path.join(paths_1.default.SRC, './icons/svgs'));
    const icons = fileNames.map(fileName => iconFactory(fileName));
    const partials = fs.readdirSync(path.join(paths_1.default.SRC, './icons/partials'));
    const partialsData = {};
