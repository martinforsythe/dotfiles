...
function systemPrune() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const terminal = create_terminal_1.createTerminal("docker system prune");
        const semver = utils_1.getCoreNodeModule('semver');
        try {
            if (configOptions.get('promptOnSystemPrune', true)) {
                let res = yield vscode.window.showWarningMessage('Remove all unused containers, volumes, networks and images (both dangling and unreferenced)?', { title: 'Yes' }, { title: 'Cancel', isCloseAffordance: true });
